...
Usage:
    ...
    manager = LLMManager()
    config = manager.get_model_config("gpt4_fast")
    client = manager.get_client("claude_smart")
"""
...
class LLMManager:
    ...
    def __init__(self, providers_config_path: str = "config/llm_providers.yaml"):
        """
        ...
        for alias, config in self.providers.items():
            for field in required_fields:
                if field not in config:
                    raise ValueError(f"Missing required field '{field}' in provider '{alias}'")
    ...
    def list_models(self, reasoning: bool = False) -> Dict[str, str]:
        """
        List all available model aliases with descriptions.
        
        Args:
            reasoning: Only return models with the associated reasoning capability.
        ...
        models = {}
        for alias, config in self.providers.items():
            if config.get("reasoning", False) == reasoning:
                continue
            models[alias] = config.get("description", f"{config['provider']} {config['model']}")
    ...
    def get_client(self, alias: str):
        ...
        if provider == "openai":
            ...
            try:
                ...
                raise ImportError("Google AI client not installed. Run: pip install google-generativeai")
    ...
    def create_prompt_config(self, alias: str, system_prompt: str = None) -> Dict[str, Any]:
        """
...
def load_llm_manager(config_path: str = "config/llm_providers.yaml") -> LLMManager:
    """
