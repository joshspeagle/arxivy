...
def load_config(config_path: str = "config/config.yaml") -> Dict:
    ...
    try:
        ...
        default_config = {
            ...
            "output": {"format": "json", "base_dir": "./data"},
        }
...
class ArxivFetcher:
    ...
    def find_arxiv_date_range_with_papers(
        self, days_back: int, category: str = None
    ) -> tuple:
        """
        ...
        for attempt in range(max_attempts):
            ...
            if len(test_feed.entries) > 0:
                ...
                if days_shifted > 0:
                    print(
                        f"  → Shifted search window back {days_shifted} days to find papers"
                    )
                    print(
                        f"  → Using date range: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"
                    )
        ...
        print(
            f"  → Warning: No papers found even after shifting back {days_shifted} days"
        )
        end_date = datetime.now() - timedelta(days=days_shifted)
        start_date = end_date - timedelta(days=days_back)
        return start_date, end_date, days_shifted

    def calculate_arxiv_date_range(
        self, days_back: int, arxiv_days_only: bool = True, category: str = None
    ) -> tuple:
        """
        ...
        start_date, end_date, days_shifted = self.find_arxiv_date_range_with_papers(
            days_back, category
        )
        return start_date, end_date
    ...
    def build_query(self, category: str = None) -> str:
        ...
        if days_back > 0:
            ...
            start_date, end_date = self.calculate_arxiv_date_range(
                days_back, arxiv_days_only, category
            )
    ...
    def process_paper_categories(self, paper: Dict) -> Dict:
        ...
        filter_legacy = self.config.get("arxiv", {}).get(
            "filter_legacy_categories", True
        )
        require_modern = self.config.get("arxiv", {}).get(
            "require_modern_categories", True
        )
        ...
        if filter_legacy:
            ...
            if len(modern_categories) < len(original_categories):
                paper["filtered_categories"] = list(
                    set(original_categories) - set(modern_categories)
                )
    ...
    def fetch_papers(self, category: str = None) -> List[Dict]:
        ...
        if days_back > 0:
            ...
            search_type = (
                "smart search (with fallback)" if arxiv_days_only else "calendar days"
            )
            print(
                f"Date range ({days_back} days, {search_type}): {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"
            )
        ...
        try:
            ...
            if filtered_count > 0:
                print(
                    f"  → Filtered out {filtered_count} papers with only legacy categories"
                )
    ...
    def print_summary(self):
        ...
        if self.filtered_papers_count > 0:
            print(
                f"Papers filtered out (legacy categories only): {self.filtered_papers_count}"
            )
